...
int main() {
    ...
    for (int epoch = 0; epoch < PWLEpochs; ++epoch) {
        ...
        printf("PWL training epoch %d, RRMSE %6.4f\r", epoch, error);
    ...
    for (int i = 0; i < nModels; ++i) {
        splineaddends.push_back(std::make_unique<KANAddend>(xmin, xmax,
            targetMin / nModels, targetMax / nModels, innerBasis, outerBasis, nFeatures));
    }
    ...
    for (int step = 0; step < SplineEpochs; ++step) {
        ...
        for (int i = 0; i < nRecords; ++i) {
            ...
            double residual = dataHolder->target[i] - model;
            for (int j = 0; j < nModels; ++j) {
                splineaddends[j]->UpdateUsingMemory(residual * muSP);
            }
        ...
        printf("Spline training epoch %d, RRMSE %6.4f\r", step, error2);
    }
    printf("\n");
