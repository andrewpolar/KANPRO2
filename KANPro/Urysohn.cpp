...
void Urysohn::UpdateUsingMemory(double delta) {
	for (int i = 0; i < _nPoints; ++i) {
		_univariateList[i].get()->UpdateUsingMemory(delta);
	}
}

void Urysohn::UpdateUsingInput(double delta, std::unique_ptr<double[]>& inputs) {
	for (int i = 0; i < _nPoints; ++i) {
		_univariateList[i].get()->UpdateUsingInput(inputs[i], delta);
	}
}
